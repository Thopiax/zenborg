...
describe("selectPeopleToReach", () => {
  ...
  const yoga: Habit = {
    ...person({ id: "h-yoga", name: "Yoga", rhythm: WEEKLY }),
    kind: undefined,
  };
  ...
  it("5. excludes someone already arranged — a future moment silences the nag", () => {
    ...
    expect(names(selectPeopleToReach(HABITS, withoutErinsPlan, NOW))).toContain(
      "Erin",
    );
  });
  ...
  it("7. excludes an archived person who would otherwise qualify", () => {
    ...
    expect(names(selectPeopleToReach(unarchived, MOMENTS, NOW))).toEqual([
      "Gina",
    ]);
  });
  ...
  it("9. filters by tag, and by areaId", () => {
    expect(
      names(selectPeopleToReach(HABITS, MOMENTS, NOW, { tag: "bcn" })),
    ).toEqual(["Carol", "Dave"]);
    expect(
  ...
  it("9b. survives a hand-edited vault whose person is missing tags entirely", () => {
    const untagged = vault({
      ...carol,
      tags: undefined as unknown as string[],
    });
    expect(() =>
      selectPeopleToReach(untagged, MOMENTS, NOW, { tag: "bcn" }),
    ).not.toThrow();
  ...
  it("10. limit truncates to the MOST overdue, not an arbitrary prefix", () => {
    expect(
      names(selectPeopleToReach(HABITS, MOMENTS, NOW, { limit: 2 })),
    ).toEqual(["Carol", "Dave"]);
    expect(
      names(selectPeopleToReach(HABITS, MOMENTS, NOW, { limit: 3 })),
    ).toEqual(["Carol", "Dave", "Hugo"]);
  });

  it("11. orders the whole set most-overdue-first", () => {
    expect(queue().map((r) => r.overdueRatio)).toEqual([
      null,
      null,
      8.57,
      4.29,
