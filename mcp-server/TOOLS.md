...
| `seed_host_blocks` | `returnsTo, unlockNote, hosts?, resolverProfile?` | Write the drogue's seed blocklist into `fences` as rules. Idempotent — ids derive from host + enforcement point, so re-running replaces; fences it did not write are untouched. One write, not one per host. |
