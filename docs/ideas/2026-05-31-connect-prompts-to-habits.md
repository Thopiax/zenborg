...
- **First instance shipped (read-only).** `Fitness → Garmin`. Convention: an area or habit tagged **`integrates-<system>`** (hyphen — zenborg's tag sanitizer strips `:`). Wired into `week-review` (planned-vs-actual reconciliation + a load note into lessons) and `week-planning` (readiness / TSB / sleep → the capacity read). Activity→habit matching is conversational (propose-from-stats: easy HR → recovery run, distance → long run, intervals → speed), never hardcoded.
