...
The original count filtered on `archived`. The field is **`isArchived`**, so the query matched nothing and silently included archived records. Measured live: the Family, Friends and Sensitive areas hold 50 records, of which 7 are archived (`Joanna`, `Lena`, `Sophie`, `Thiago`, `colloc auber`, `dad`, `family breakfast`), leaving 43 live. Two are rituals, so 41 get marked and the two fused pairs split into four — **43 people**.
...
**Split siblings.** `Yaya` and `Paul` keep their original UUIDs, so their history stays attached. `Abuelo` and `Mari` are new records with fresh UUIDs, `emoji: null` (falling back to the area emoji rather than inheriting a wrong one), and no history — so they arrive never-contacted and sit at the head of the queue until seen.
