...
- Produces (later tasks rely on these exact names):
  ...
  - `interface ExternalRef { source: "eventkit"; eventId: string; calendarId: string; lastWrittenHash: string; lastSyncedAt: string }`
...
export interface ExternalRef {
  ...
  /**
   * Hash of the event fields as of the last sync, in either direction:
   * what zenborg last pushed to its own calendar, or what it last ingested
   * from a foreign one. The echo-suppression comparand (spec D4).
   */
...
- Produces (Tasks 4, 5, 13 rely on these exact shapes):
  ...
  - `momentHash(fields: EventFields): string` (16-char lowercase hex, FNV-1a 64 over `` `${day}|${startTime}|${durationMin}|${title}` ``)
...
describe("momentHash", () => {
  it("is stable: same fields, same hash", () => {
    expect(momentHash(fields)).toBe(momentHash({ ...fields }));
  });
  ...
  it("changes when any pushed field changes", () => {
    const base = momentHash(fields);
    expect(momentHash({ ...fields, startTime: "10:45" })).not.toBe(base);
    expect(momentHash({ ...fields, durationMin: 45 })).not.toBe(base);
    expect(momentHash({ ...fields, title: "sync" })).not.toBe(base);
    expect(momentHash({ ...fields, day: "2026-08-25" })).not.toBe(base);
  });
});
...
Run: `pnpm vitest run src/domain/services/__tests__/CalendarSyncService.test.ts`
Expected: PASS (19 vector cases + 4 apply tests + Task 3's 7).
...
describe("properties", () => {
  it("publish then ingest is identity on day, startTime and durationMin", () => {
    fc.assert(
      fc.property(gridTime, gridDuration, isoDay, (startTime, durationMin, day) => {
        const m = newMoment({ day, startTime, durationMin, phase: null });
        const fields = eventFieldsForMoment(m);
        expect(fields).not.toBeNull();
        // Ingest the published event back through the reconciler's snap path.
        const snapped = snapToGrid(fields!.startTime, fields!.durationMin);
        expect(snapped).toEqual({ startTime, durationMin });
        expect(fields!.day).toBe(day);
        // And the echo hash agrees with itself.
        expect(momentHash(fields!)).toBe(momentHash(eventFieldsForMoment(m)!));
      }),
    );
  });
...
export interface WeekGridBlock {
  ...
  readonly durationMin: number;
  readonly topPct: number; // 0..100 within the grid's hour span
...
describe("deriveWeekGridViewModel", () => {
  ...
  it("positions a timed moment by start and duration", () => {
    ...
    // 6..22 is a 16 hour span; 08:00 sits 2h in: 12.5%; 60min: 6.25%
    expect(block.topPct).toBeCloseTo(12.5);
    expect(block.heightPct).toBeCloseTo(6.25);
  });
...
Pure function, no observables. Derive the seven dates from `weekStart` with the date helpers in `src/lib/dates.ts` (see `fromISODate` usage in `BandedHeatmap.tsx`; avoid `new Date(iso)` UTC parsing). Bucket moments by `day`; timed ones (with `startTime`) become blocks (`topPct = (startMinutes - startHour * 60) / span * 100`, clamped to `[0, 100]`; `heightPct = durationMin / span * 100` with `durationMin ?? 60`); ambient allocated ones sort by phase `order` (via config lookup) then `order` and land in `ambient`. Follow the structure of `deriveBandedHeatmapViewModel` in the same directory.
...
const vm: WeekGridViewModel = {
  days: [
    day("2026-08-24", [
      block({}),
      block({ momentId: "m-2", name: "dentist", areaId: "a2", tentative: true, topPct: 50 }),
    ]),
...
describe("WeekGrid", () => {
  ...
  it("renders one hour rule per entry in vm.hours", () => {
    render(<WeekGrid vm={vm} areas={areas} onAccept={noop} onRename={noop} />);
  ...
  it("renders more than three blocks in one phase band", () => {
    const five = ["12:00", "13:00", "14:00", "15:00", "16:00"].map((t, i) =>
      block({ momentId: `m-${i + 10}`, name: `b${i}`, startTime: t, topPct: 40 + i * 5 }),
    );
...
- `WeekGrid.tsx`: CSS grid, one column of hour labels + seven day columns; hour rules as absolutely positioned hairlines (`border-stone-200 dark:border-stone-700`); each day column header shows weekday + date in stone; today's column gets the subtle highlight treatment `TimelineCell` uses for its Today column. The ambient lane is a thin strip under the day header, above the hour rows, rendering ambient moments as small labelled chips (area-colored dot + name), read-only in this task.
- `WeekGridDayColumn.tsx`: relative-positioned container; blocks absolutely positioned by `topPct`/`heightPct`.
...
it("fnv1a64 pins the cross-language digests the Swift port must match", () => {
  ...
  expect(fnv1a64("2026-08-24|10:30|30|standup")).toBe("19f4ed4a0983eb05");
  expect(momentHash(fields)).toBe("19f4ed4a0983eb05");
});
```
...
./calendar-sidecar/dist/zenborg-calendar hash "2026-08-24|10:30|30|standup"  # expect 19f4ed4a0983eb05
```
...
- [ ] **Step 3: Run the self-test and verify all 19 vectors pass**
...
Expected: `19 passed, 0 failed`. A failure means the port diverged; fix the Swift, never the vectors (the TS tests own them).
...
- Produces: the five sidecar responsibilities from the spec, and nothing else:
  ...
  5. **Write**: apply `ReconcileAction`s through `Vault.swift`; after every applied action, set `externalRef.lastWrittenHash` to the hash of the event's **current** fields and `lastSyncedAt` to now; log every `overwroteMomentEdit`/`overwroteEventEdit` loss to stderr with both timestamps (row R12: log the loss).
- Reconcile pass structure (`reconcile-once` and each watch tick): read vault + config, fetch events (Zenborg calendar + selected calendars, window), pair by `externalRef.eventId`, call `reconcile` per pair (moments without events, events without moments, matched pairs), apply actions, write moments once (single atomic write per pass; the app's watcher then emits one `vault:collection-changed` and the running UI reloads: the path that already exists, spec D7).
...
11. With the app running, a sidecar vault write triggers `vault:collection-changed` and the week grid updates without a reload.
...
1. **Ingested moment names vs the 1-3 word invariant.** Event titles are arbitrary ("Standup with the data team"); moment names cap at 3 words. The plan derives the moment name from the first 3 words of the title and never syncs names back on later edits (`applyEventToMoment` updates time only). Confirm, or name a different derivation.
...
6. **reconcile() lives twice** (TS as the specification with the full vitest suite; Swift as a mechanical port gated by the same 19 JSON vectors in the build script). The spec implies but never states this duplication; flag if you would rather the sidecar shell out to a compiled TS reconciler instead.
