...
**Bet:** Harvest renders one closed season — its intention, the moments you planted, a reflection written from both those moments and whatever traces you left, the photographs you took — and never grades it.
...
4. **The reflection reads intention, not only journals** (`scripts/summarize-cycles.mjs:57`). The corpus becomes a compact digest of the season's moments — name, area, people, phase — plus whatever journal entries exist. A season with no journals still reflects, from what you planted. This is what lets the prose hold both sides, and the digest sits on the aggregate tier where journal prose is raw. Ollama stays opt-in: absent, harvest is still whole.
...
- Rebuilding extraction as a wake blueprint now. `by_cycle_window` indexing does not exist in penceive-core. Slice 4's boundary — it writes `Cycle.reflection` and nothing else — is what makes waiting cheap.
...
- **"I meant X, the season held Y" is one step from "you failed at X."** Moments in the corpus is what makes the reflection worth reading and is also the nearest path to a verdict. The prompt narrates what the season held; it never scores intention against outcome.
- `~/.wake/blueprints/cycle.yaml` has no moments source — only `type: entry` from ponds. Slice 4 amends a draft contract; do it there, not in the script alone.
- `cycle.yaml` pins `qwen3.6:35b`; the benchmark says `qwen3:4b` at 6.7s/cycle is indistinguishable. Amend or record why not.
...
2. A season with zero journal entries produces a reflection drawn from its moments, instead of being skipped.
3. A machine-drafted reflection is visually distinguishable from a human-written one. Editing stamps `reflectionSource: "human"`; re-running the summarizer leaves it alone.
4. No surface in harvest ranks, scores, compares, or shows a percentage against a target — including the reflection prose.
5. Harvest loads with the network off. No external asset fetch.
6. `pnpm test` green, including the collections-sync parity test with `reflectionSource` added.
7. With zenborg uninstalled the record is still readable: reflections are plain text in `cycles.json`, photographs untouched in the user's own library.

---

_Supersedes `2026-08-14-harvest-the-season-reads-back.md`. Drafted by Claude (scribe)._
