...
describe("personHealth", () => {
  ...
  it("counts a moment shared with several people for each of them", () => {
    ...
    expect(personHealth(p, ms, NOW)).toBe("blooming");
    expect(personHealth(person({ id: "p-fox", rhythm: { period: "weekly", count: 1 } }), ms, NOW)).toBe("blooming");
  });
});
```
...
- `Ben & Dee` → rename to `Ben` (keeps id), create `Dee` (new UUID, same areaId, same tags, same rhythm, `order` = max order in that area + 1)
- `Ada & Cal` → rename to `Ada` (keeps id), create `Cal` (same rule)
...
for n in ['Ben', 'Dee', 'Ada', 'Cal']:
    assert n in names, f'missing {n}'
for n in ['Ben & Dee', 'Ada & Cal', 'family breakfast', 'colloc auber', 'poetry', 'tantric']:
