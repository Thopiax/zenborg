...
function HabitStack({
  ...
  return (
    <div className="flex flex-col">
      {/* Stack layers — thin bars peeking above the card */}
      ...
      {/* Expanded variants — indented with left border for hierarchy */}
      {expanded && (
        <div className="flex flex-col gap-1 mt-2 mb-2 ml-4 mr-1">
          {childHabits.map((child) => (
            <button
              ...
              className={cn(
                "flex items-center gap-2 px-3 py-3 rounded-md transition-all hover:ring-2 hover:ring-offset-2 ring-offset-transparent text-left w-full",
                "border-l-[3px] border-black/15 dark:border-white/15",
              )}
              style={{
                backgroundColor: areaColor,
                "--tw-ring-color": areaColor ? `${areaColor}99` : undefined,
          ...
          {/* Collapse — caret-up at bottom center */}
          <button
            type="button"
            onPointerDown={(e) => e.stopPropagation()}
            onClick={(e) => {
            ...
            className="mx-auto mt-1 p-0.5 rounded text-stone-400 dark:text-stone-500 hover:text-stone-600 dark:hover:text-stone-300 transition-colors"
