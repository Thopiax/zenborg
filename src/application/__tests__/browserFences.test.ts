...
import { DROGUE_SEED_HOSTS } from "@/domain/intervention/rules/hostBlock";
...
describe("seedHostBlocks", () => {
  it("writes the seed blocklist as fences, one rule per host", async () => {
    ...
    expect(result.declared).toHaveLength(DROGUE_SEED_HOSTS.length);
    const domains = Object.values(stored()).map(
      (r) => (r.scope as { domain: string }).domain,
    );
    expect(domains.sort()).toEqual([...DROGUE_SEED_HOSTS].sort());
  });

  it("is idempotent — re-seeding replaces rather than accumulating", async () => {
    const { d, stored } = deps();
    await seedHostBlocks(d, { returnsTo: ["Craft"], unlockNote: "n" });
    await seedHostBlocks(d, { returnsTo: ["Craft"], unlockNote: "n" });
    expect(Object.keys(stored())).toHaveLength(DROGUE_SEED_HOSTS.length);
  });
