 ...
 *
 * zenborg owns moments and knows nothing about friction. the extension reads
 * them and decides. `allow` may be *seeded* from a moment's `refs` (the URLs
 * it points at) — that seeding is an extension-side derivation, named and
 * separable, and it is not a meaning of `refs`. Same one-way seam as the area
 * list: zenborg edits, the extension mirrors, never the reverse.
 *
 * ── What this is not ────────────────────────────────────────────────────
 *
 ...
 * low. It never escalates to a hard block, because the highest friction the
 * extension is willing to own is a gate you can walk through, and making it impossible to
