...
describe("isSettlement — the page reports its own outcome, and is not trusted", () => {
  it("accepts a well-formed settlement", () => {
    expect(isSettlement({ type: "kairos-intervention-settled", ruleId: "r", proceeded: true })).toBe(
      true
    );
  });
  ...
  it("rejects anything else", () => {
    ...
    expect(isSettlement({ type: "kairos-gate-check" })).toBe(false);
    expect(isSettlement({ type: "kairos-intervention-settled", ruleId: 7, proceeded: true })).toBe(
      false
    );
    expect(isSettlement({ type: "kairos-intervention-settled", ruleId: "", proceeded: true })).toBe(
      false
    );
    expect(isSettlement({ type: "kairos-intervention-settled", ruleId: "r" })).toBe(false);
  });

  it("rejects a rule id long enough to be a payload rather than an id", () => {
    expect(
      isSettlement({ type: "kairos-intervention-settled", ruleId: "x".repeat(129), proceeded: true })
    ).toBe(false);
  });
});
