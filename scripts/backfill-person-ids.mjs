...
if (people.length === 0) {
  console.error('No habits with kind:"person" found — has the people migration run?');
  process.exit(1);
}
...
for (const p of people) {
  for (const name of [p.name, ...(p.aliases ?? [])]) byKey.set(slug(name), p.id);
}
// word-boundary regex per person, longest names first so "Jean Michel" wins over "Jean"
const namePatterns = people
  .flatMap((p) => [p.name, ...(p.aliases ?? [])].map((n) => ({ name: n, id: p.id })))
...
for (const m of Object.values(momentsById)) {
  ...
  momentsById[m.id] = { ...m, personIds: Array.from(found), updatedAt: new Date().toISOString() };
  touched += 1;
  if (!APPLY) {
    const names = Array.from(found).map(
      (id) => people.find((p) => p.id === id)?.name ?? id,
    );
    console.log(`  + ${m.day ?? '(unallocated)'} ${m.name} ← ${names.join(', ')}`);
  }
}

console.log(`\n${APPLY ? 'Applied' : 'Dry-run'} against ${VAULT}`);
console.log(`  ${touched} moments${APPLY ? ' written' : ' would change'} (${viaTag} links via person- tags, ${viaName} via name match)`);
