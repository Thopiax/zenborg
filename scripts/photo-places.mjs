...
const LIB = path.join(os.homedir(), 'Pictures/Photos Library.photoslibrary/database/Photos.sqlite');
...
for (const ext of ['-wal', '-shm']) {
  try { fs.copyFileSync(LIB + ext, TMP + ext); } catch { /* absent is fine */ }
}
const rows = execFileSync('sqlite3', [TMP, `
  ...
  .trim().split('\n').filter(Boolean)
  .map((l) => { const [day, lat, lng] = l.split('|'); return { day, lat: +lat, lng: +lng }; });
for (const ext of ['', '-wal', '-shm']) { try { fs.unlinkSync(TMP + ext); } catch {} }

const cycles = Object.values(JSON.parse(fs.readFileSync(path.join(VAULT, 'cycles.json'), 'utf8')))
  .sort((a, b) => a.startDate.localeCompare(b.startDate));
let mapping = {};
try {
  mapping = JSON.parse(fs.readFileSync(path.join(import.meta.dirname, 'backfill-tags.local.json'), 'utf8'));
} catch {}

if (process.argv.includes('--unknown')) {
  const far = rows.map((r) => ({ ...r, n: nearest(r.lat, r.lng) })).filter((r) => r.n.d > SNAP_KM);
  const clusters = new Map();
  for (const r of far) {
    const key = `${r.lat.toFixed(1)},${r.lng.toFixed(1)}`;
    const c = clusters.get(key) ?? { n: 0, first: r.day, last: r.day, near: r.n.name, d: Math.round(r.n.d) };
    c.n += 1;
  ...
  for (const [k, c] of [...clusters.entries()].sort((a, b) => b[1].n - a[1].n).slice(0, 25))
    console.log(`  ${String(c.n).padStart(4)} photos  ${c.first} → ${c.last}  (${c.d}km from ${c.near})  @${k}`);
  console.log();
  process.exit(0);
}
...
let agree = 0, disagree = 0, unknownClaim = 0, noPhotos = 0;
const conflicts = [];
for (const c of cycles) {
  ...
  if (!tally.size) { noPhotos += 1; continue; }
  ...
  if (!claimed) {
    unknownClaim += 1;
    conflicts.push({ kind: 'UNCLAIMED', c, says: coords[topTag].name, n: topN, total });
  } else if (claimed === topTag) {
    agree += 1;
  } else {
    disagree += 1;
    conflicts.push({ kind: 'CONFLICT', c, claims: coords[claimed].name, says: coords[topTag].name, n: topN, total });
  }
}

console.log(`\nPHOTO GPS vs CYCLES — ${rows.length} located photos, ${byDay.size} days\n`);
console.log(`  ${agree} cycles agree · ${disagree} conflict · ${unknownClaim} unclaimed but photos know · ${noPhotos} no photos\n`);
for (const x of conflicts.slice(0, 30)) {
  const w = `${x.c.startDate}→${x.c.endDate ?? 'open'}`.padEnd(24);
  if (x.kind === 'CONFLICT')
    console.log(`  ⚠ ${w} "${x.c.name}" claims ${x.claims}, photos say ${x.says} (${x.n}/${x.total})`);
  else
    console.log(`  + ${w} "${x.c.name}" has no place; photos say ${x.says} (${x.n}/${x.total})`);
}
console.log();
