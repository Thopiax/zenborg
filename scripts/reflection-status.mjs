...
const lens = withRefl.map((c) => c.reflection.trim().length).sort((a, b) => a - b);
const l0words = withRefl.map((c) => c.reflection.split('\n')[0].trim().split(/\s+/).length);
...
console.log(`  ${withRefl.length} of ${cycles.length} cycles (${Math.round((withRefl.length / cycles.length) * 100)}%)`);
console.log(`  ${twoRung.length} carry both rungs (L0 sentence + L1 tldr)\n`);
for (const [k, e] of byEra)
  console.log(`  ${k.padEnd(22)} ${String(e.done).padStart(3)} / ${String(e.total).padEnd(4)} ${'█'.repeat(Math.round((e.done / e.total) * 20)).padEnd(20, '·')}`);
...
console.log(`  L0 shape: ${leak} label leaks · ${multi} multi-sentence · ${long} over 28 words`);
...
for (const c of withRefl.slice(-8).reverse())
  console.log(`    ${c.startDate}  ${c.name.padEnd(22)} ${String(c.reflection.trim().length).padStart(4)} chars`);
