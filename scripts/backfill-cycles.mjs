...
const CSV_PATH =
  csvArg !== -1
    ...
    : path.join(import.meta.dirname, "travel-log.local.csv");
if (!fs.existsSync(CSV_PATH)) {
  console.error(
    `No travel log at ${CSV_PATH} — copy the export there or pass --csv <path>.`,
  );
  process.exit(1);
}
...
let created = 0,
  ...
  skippedDup = 0;
for (const r of rows) {
  if (r.end >= firstRealCycle) {
    skippedOverlap += 1;
    continue;
  }
  if (existingStarts.has(r.start)) {
    skippedDup += 1;
    continue;
  }
...
console.log(
  `  ${created} historical cycles${APPLY ? " written" : " would be created"} (era before ${firstRealCycle})`,
);
console.log(
  `  ${skippedOverlap} rows overlap the zenborg era (place evidence only), ${skippedDup} already present`,
);
