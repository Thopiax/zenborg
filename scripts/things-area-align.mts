#!/usr/bin/env node
/**
 * things-area-align — align Things 3 areas with zenborg areas.
 *
 * Reads both taxonomies live:
 *   - zenborg areas from $ZENBORG_HOME/areas.json (default ~/.zenborg)
 ...
 * database (mode=ro) and never touches any other file under ~/.zenborg.
 *
 * Usage:
...
const defaultMapPath = (): string => join(vaultHome(), "things-area-map.json");

const readZenborgAreas = (): readonly ZenborgArea[] => {
  const path = join(vaultHome(), "areas.json");
  if (!existsSync(path)) {
    throw new Error(`zenborg areas not found at ${path} (set ZENBORG_HOME?)`);
  }
...
const mergeMap = (
  ...
  return {
    note:
      existing?.note ??
      seed?.note ??
      'Things 3 area uuid -> zenborg area id. Hand-editable. "//" is a label, regenerated by scripts/things-area-align.mts --write. "?" means undecided: set "zenborg" then delete the "?" line. zenborg:null with no "?" means deliberately unmapped.',
...
const renderText = (
  ...
  line("=================");
  line(
    `  zenborg areas  ${liveZenborg.length} live (${zenborg.length - liveZenborg.length} archived)  ${join(vaultHome(), "areas.json")}`,
  );
  line(
  ...
  if (values.help === true) {
    process.stdout.write(
      [
        ...
        "  --map <path>      mapping file (default $ZENBORG_HOME/things-area-map.json)",
