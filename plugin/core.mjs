...
// extension's domain (apps/extension/modules/domain): this file deploys standalone
...
/** @typedef {{ sessionStartTs: number, lastPromptTs: number, inferNudgedTs: number, granularity: string, granularityDay: string, focus: boolean, focusTs: number, lastRuleHash: string, consentShownTs: number, lastMomentId: string, granularitySeen?: Record<string, GranularitySeen>, disclosureShownTs?: number }} State */
 ...
 *
 * Normalized like `createDomain` in apps/extension/modules/domain/value-objects.ts
 ...
 * am I doing right now" would let two readers disagree, which is the
...
// Events mirror the `ActivityEvent` in apps/extension/modules/domain. The log is
// the product; these
...
export function consentLines() {
  return [
    ...
    "[keel] · New here? `keel onboard`: what you are asked to allow, and the two questions a rule needs answered.",
  ];
}
