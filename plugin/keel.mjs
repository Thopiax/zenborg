...
// ── Onboarding ────────────────────────────────────────────────
// `keel onboard`: the disclosure, the preflight, and two questions. It asks and
// it reports; it writes nothing to the kernel's collections. Areas and cycles
// have one writer and it is zenborg, so the answers land there (the garden, or
// the agent through zenborg's MCP) and onboarding reads the result back.
//
// Re-runnable on purpose. Run before answering it names what is still open;
// run after, it names the `serves` a rule can now carry.
async function cmdOnboard(now, arg) {
  const forced = String(arg ?? "") === "--disclosure";
  const state = loadState();
  const first = !state.disclosureShownTs;

  const probe = await probeMachine({ now });
  const lines = onboardLines({
    vault: { areas: loadAreas(), cycles: loadCycles() },
    probe,
    showDisclosure: first || forced,
  });
...
async function main() {
  ...
  if (cmd === "onboard") return cmdOnboard(now, sub);
