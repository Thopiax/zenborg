...
import {
  QUESTIONS, openQuestions, readiness, disclosureLines,
  preflight, preflightLines, onboardLines,
} from "./onboard.mjs";
...
const PROBE = {
  vaultPath: "/tmp/vault", vaultWritable: true,
  logEventsToday: 0,
  screenRecording: null, screenRecordingProcess: "osascript",
  nativeHostProfiles: [],
};
...
test("onboarding asks exactly two things, and stops", () => {
  ...
  assert.deepEqual(QUESTIONS.map((q) => q.id), ["plots", "intention"]);
});

test("a blank vault leaves both open, plots first", () => {
  assert.deepEqual(openQuestions(BLANK).map((q) => q.id), ["plots", "intention"]);
});

test("plots alone leaves the season intention open", () => {
  const r = { areas: [{ id: "a1", name: "craft" }], cycles: [{ id: "c1", name: "First Cycle", intention: null, startDate: "2026-08-01", endDate: null }] };
  assert.deepEqual(openQuestions(r).map((q) => q.id), ["intention"]);
  assert.equal(readiness(r).ready, false);
});

test("an intention alone leaves the plots open: a DistalRef needs both halves", () => {
  const r = { areas: [], cycles: [{ id: "c1", name: "First Cycle", intention: "ship the thing", startDate: "2026-08-01", endDate: null }] };
  assert.deepEqual(openQuestions(r).map((q) => q.id), ["plots"]);
  assert.equal(readiness(r).ready, false);
});

test("both answered: nothing is left to ask and serves becomes inhabitable", () => {
  const r = {
    areas: [{ id: "a1", name: "craft" }, { id: "a2", name: "body" }],
    cycles: [{ id: "c1", name: "First Cycle", intention: "ship the thing", startDate: "2026-08-01", endDate: null }],
  };
...
test("a closed season does not count: the intention has to be this season's", () => {
  const r = {
    ...
    cycles: [{ id: "c0", name: "Last", intention: "old", startDate: "2026-01-01", endDate: "2026-02-01" }],
  };
  assert.equal(readiness(r).ready, false);
  assert.deepEqual(openQuestions(r).map((q) => q.id), ["intention"]);
});
...
function everythingOnboardingSays() {
  const ready = {
    areas: [{ id: "a1", name: "craft" }],
    cycles: [{ id: "c1", name: "First Cycle", intention: "x", startDate: "2026-08-01", endDate: null }],
  };
  return [
    ...disclosureLines(),
...
test("onboarding offers no plot, no habit and no intention of its own", () => {
  ...
  for (const q of QUESTIONS) {
    assert.equal("suggestions" in q, false, `question "${q.id}" ships candidate answers`);
    assert.equal("default" in q, false, `question "${q.id}" ships a default answer`);
  }
  assert.doesNotMatch(said, /\be\.g\.\s+(work|health|family|craft|body|fitness|learning)\b/i);
});
...
test("the disclosure states what is recorded and what is not", () => {
  const d = disclosureLines().join("\n");
  assert.match(d, /256/);
  assert.match(d, /never page contents, never URLs, never prompts, never keystrokes/i);
...
test("the preflight names every grant before anything is asked", () => {
  ...
  assert.deepEqual(ids, [
    "vault", "hooks", "login-items", "screen-recording",
    "native-host", "incognito",
  ]);
...
test("a missing screen-recording grant names the silent failure", () => {
  const row = preflight({ ...PROBE, screenRecording: false })
    .find((g) => g.id === "screen-recording");
...
test("the preflight runs before the questions, in the rendered order", () => {
  const out = onboardLines({ vault: BLANK, probe: PROBE, showDisclosure: true }).join("\n");
  ...
  assert.ok(disclosureAt >= 0 && preflightAt > disclosureAt && questionAt > preflightAt,
...
function keel(home, ...args) {
  ...
  return spawnSync(process.execPath, [KEEL, ...args], { encoding: "utf8", env });
}
...
function answerAsZenborgWould(home) {
  writeFileSync(join(home, "areas.json"), JSON.stringify({
    "area-1": { id: "area-1", name: "one", order: 0 },
  }));
  writeFileSync(join(home, "cycles.json"), JSON.stringify({
    "cycle-1": {
      id: "cycle-1", name: "First Cycle", startDate: "2026-08-01", endDate: null,
...
test("a blank vault reaches a state where a rule can be authored", () => {
  ...
  try {
    ...
    assert.match(before.stdout, new RegExp(QUESTIONS[0].ask.slice(0, 30).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
    assert.match(before.stdout, new RegExp(QUESTIONS[1].ask.slice(0, 30).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
    ...
    for (const f of ["areas.json", "cycles.json", "moments.json", "habits.json", "phaseConfigs.json"]) {
