...
/** Cycles from the kernel: the seasons, and the only place an intention is
 * declared. Zenborg is the only writer, as with areas; onboarding reads them to
 * find out whether `serves: DistalRef` is inhabitable yet, and writes nothing.
 *
 * Fails soft to `[]`, so an unreadable vault reads as "not set up" rather than
 * throwing at a peer who has not opened the app yet.
 * @returns {{id: string, name: string, intention: string|null, startDate: string, endDate: string|null}[]} */
export function loadCycles() {
  ...
  return list
    ...
    .map((c) => ({
      id: String(c.id),
      name: String(c.name ?? ""),
      intention: typeof c.intention === "string" ? c.intention : null,
...
// What this surface can honestly find out about the machine. Anything it cannot
// find out is reported as unmeasurable by the pure layer rather than guessed at
// here, which is why every probe below returns `null` on failure instead of a
// default that would read as an answer.

/** Chromium reads native-messaging manifests from Chrome's directory even when
 * the browser is Brave, which is why the installer writes only there. Both are
 * checked anyway: a peer on plain Chromium or Edge has a different path and
 * would otherwise be told the relay is missing when it is merely elsewhere. */
const NATIVE_HOST_DIRS = [
  ["Chrome", "Library/Application Support/Google/Chrome/NativeMessagingHosts"],
  ["Chromium", "Library/Application Support/Chromium/NativeMessagingHosts"],
  ["Edge", "Library/Application Support/Microsoft Edge/NativeMessagingHosts"],
];
const NATIVE_HOST_NAME = "tech.equanimi.keel";

/** Can the vault actually be written? Asked by writing, because `existsSync`
 * and a mode bit both answer a different question than the one that matters. */
export function vaultWritable() {
  const probe = join(KAIROS_DIR, `.keel-onboard-probe.${process.pid}`);
  try {
 ...
 * prompts, which is what makes it usable before a peer has consented to
 * anything.
 *
 * The answer is about THIS process. macOS grants the permission per
 * application, so a `true` here says osascript has it and says nothing about
 * the app. The pure layer is what refuses to round that up, and this returns
 * the raw fact plus the name of what was measured.
 * @returns {boolean|null} null when the check could not be run at all */
 ...
 * what any of it means lives in onboard.mjs.
 * @param {{ now?: number }} [opts]
 * @returns {Promise<import("./onboard.mjs").Probe>} */
export async function probeMachine({ now = Date.now() } = {}) {
  ...
  return {
    ...
    nativeHostProfiles: nativeHostProfiles(),
  };
}
 ...
 * now" and two readers must not be able to disagree about it.
 *
